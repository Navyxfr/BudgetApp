...
function MainApp(){
  ...
  /* ── Login screen at startup (no data yet + not logged in) ── */
  if(!skipLogin&&!authUser&&(!meta||meta.households.length===0)){
    return(
      <div style={{minHeight:"100vh",background:"var(--bg)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"40px 28px",fontFamily:"-apple-system,BlinkMacSystemFont,'SF Pro Display',system-ui,sans-serif"}}>
        <style>{CSS}</style>
        <div style={{width:72,height:72,borderRadius:22,background:"linear-gradient(135deg,#C8956C,#D4A574)",display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 28px",boxShadow:"0 16px 48px rgba(200,149,108,.3)"}}><Wallet size={32} color="#fff" strokeWidth={1.5}/></div>
        <h1 style={{fontSize:28,fontWeight:700,color:"var(--text)",margin:"0 0 8px",letterSpacing:-.8}}>Budget Planner</h1>
        <p style={{fontSize:14,color:"var(--text3)",margin:"0 0 40px",textAlign:"center",lineHeight:1.5}}>Gérez votre budget de foyer simplement.<br/>Synchronisé entre tous vos appareils.</p>
        ...
        <p style={{fontSize:11,color:"var(--text4)",margin:"24px 0 0",textAlign:"center",lineHeight:1.5}}>Sans compte, vos données restent<br/>uniquement sur cet appareil.</p>
      </div>
    );
  }
        ...
        <Card>
          ...
          {authUser?(
            ...
            <div style={{display:"flex",flexDirection:"column",gap:10}}>
              ...
              <Btn full onClick={async()=>{try{const u=await window.firebaseAuth.signIn();if(u){setAuthUser(u);toast("Connecté !");}}catch(e){if(e.code!=="auth/popup-closed-by-user"&&e.code!=="auth/redirect-cancelled-by-user")toast(e.code||e.message);}}}><LogIn size={16}/>Se connecter avec Google</Btn>
            </div>
          )}
        </Card>
